...
主作者 [YangYueXiong](https://github.com/yangyuexiong) 是某测试平台V1.0与V2.0的设计者，期间充当产品，设计，研发，测试，运维等多个角色。

期间因各种因素导致最后测试平台落地的结果并不理想，但在整个过程中其受益匪浅并有所总结，而组织者 [ExileLine](https://github.com/ExileLine)
希望做出一个能让测试人员认可并真正能`降本增效`的测试平台，其言既然不能于公司中很好实践，那么就流放到整个社区，让更多的测试人员使用并提出建议使其更好发挥实际作用，因此取名`放逐`。
...
- 2020-9-18：某某测试平台V2.0（某思维，`某某测试平台V1.0`重构）
- 2021-7-20：放逐测试平台V3.0（个人开源，因开发如下V4.0，仓库关闭）
- 2022-2-23：某某测试平台V4.0（基于`放逐测试平台V3.0`扩展为某公司定制化开发）
- 2023-7-18：放逐测试平台V5.0（组织开源）
- 2025-4-17：规划使用`Fastapi`重构核心服务以及使用`Playwright`替换`Selenium`(进行中...)
