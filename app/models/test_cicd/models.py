...
class TestCiCdMap(BaseModel):
    ...
    def __repr__(self):
        return 'TestCICD 模型对象-> ID:{} project_id:{} project_name:{} app_name:{} mirror:{} url:{}'.format(
            self.id, self.project_id, self.project_name, self.app_name, self.mirror, self.url
        )
