...
class PlatformConfModel(BaseModel):
    ...
    safe_scan_url = db.Column(db.String(255), comment='安全扫描代理url')
    safe_scan_report_path = db.Column(db.String(1024), comment='安全测试报告路径(拼接server_url)')
    safe_scan_report_name = db.Column(db.String(1024), comment='安全测试报告名称(拼接safe_scan_report_path)')
